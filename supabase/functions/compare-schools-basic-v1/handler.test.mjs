...
import { BASIC_V1_FIELDS as fields } from './handler.mjs';
const row = id => ({ ...Object.fromEntries(fields.map(f => [f, null])), school_id: id });
const records = [{ ...row('nyu'), tuition_fees: 68576, first_year_enrollment: 5723, internal: 'must not leak' }, row('bu')];
...
const valid = { schoolA: 'nyu', schoolB: 'bu' };
 ...
 test('NYU + BU success, RPC arguments, allowlist, and CORS', async () => {
  const handler = createHandler({ getEnv: n => env[n], fetchImpl: async (url, options) => {
    assert.equal(url, 'https://example.invalid/rest/v1/rpc/get_school_comparison_basic_v1');
  ...
  assert.equal(data[1].tuition_fees, null);
...
test('Basic v1 preserves every field, arrays and NULLs and excludes internal/GPA values', async () => {
  const full = [{...row('ucb'), tuition_fees:58484, coa:93944, english_tests_accepted:['TOEFL','IELTS','Duolingo'], region:'west_coast', ranking_tier:'top_30', gpa_year:'private', internal:'private'}, row('nyu')];
  const handler = createHandler({getEnv:n=>env[n],fetchImpl:async()=>Response.json(full)});
  const response = await handler(request({schoolA:'ucb',schoolB:'nyu'}));
  assert.equal(response.status,200);
  const data=await response.json();
  assert.equal(Object.keys(data[0]).length,51);
  assert.equal(data[0].tuition_fees,58484); assert.equal(data[0].coa,93944);
  assert.equal(data[0].sat_25,null); assert.equal(data[0].english_policy_cycle,null);
  assert.deepEqual(data[0].english_tests_accepted,['TOEFL','IELTS','Duolingo']);
  assert.equal(Object.hasOwn(data[0],'gpa_year'),false);
  assert.equal(Object.hasOwn(data[0],'tuition'),false);
});
